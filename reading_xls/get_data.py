...
def _gsheet_export_xlsx_url(url: str) -> str:
    ...
    if not m: raise ValueError("Could not parse Google Sheet ID from URL")
...
def _download_gsheet_xlsx(url: str, cache_seconds: int = 60) -> Path:
    ...
    try:
        if tmp_path.exists() and stamp_path.exists():
            if time.time() - float(stamp_path.read_text().strip() or "0") < cache_seconds:
                logger.info("Using cached XLSX")
                return tmp_path
    except Exception: pass
    logger.info(f"Downloading XLSX from {export_url}")
    urllib.request.urlretrieve(export_url, tmp_path)
    stamp_path.write_text(str(time.time()))
    return tmp_path

def _load_excel(filename: str) -> Path:
    if _is_gsheet_url(filename): return _download_gsheet_xlsx(filename, cache_seconds=60)
    root = Path(__file__).resolve().parents[1]
    candidates = [Path(filename), root/"start"/"data"/filename, root/"data"/filename, Path("start/data")/filename]
    for p in candidates:
        if p.exists(): return p
    raise FileNotFoundError(f"Excel '{filename}' not found.")
...
def _allowed_value_converter(v):
    return [item.strip() for item in _clean_val(v).split(";") if item.strip()]

def get_data(filename: str):
    xlsx_path = _load_excel(filename)
    try: xls = pd.ExcelFile(xlsx_path, engine="openpyxl")
    except Exception as e: logger.error(f"Error opening Excel: {e}"); raise e
    ...
    for sheet_name in xls.sheet_names:
        ...
        if lower_name in ["data", "items", "trials"]:
            logger.info(f"Reading Data: {sheet_name}")
            df = xls.parse(sheet_name, dtype=str, keep_default_na=False).map(_clean_val)
            if "Item" in df.columns: df["Item"] = df["Item"].apply(lambda x: x.replace(" ", "_"))
            all_data_frames.append(df)
        elif lower_name.startswith("practice"):
            logger.info(f"Reading Practice: {sheet_name}")
            df = xls.parse(sheet_name, dtype=str, keep_default_na=False)
            cols = [str(c).lower() for c in df.columns]
            if "name" in cols and "value" in cols:
                practice_conf = {}
                name_idx, val_idx = cols.index("name"), cols.index("value")
                for _, row in df.iterrows():
                    k, v = _clean_val(row.iloc[name_idx]), _clean_val(row.iloc[val_idx])
                    if k: practice_conf[k] = v
                
                nums = re.findall(r"\d+", sheet_name)
                clean_key = f"Practice{nums[0]}" if nums else sheet_name.strip().capitalize()
                settings_dict[clean_key] = practice_conf

    if not all_data_frames: raise ValueError("No Data sheet found")
