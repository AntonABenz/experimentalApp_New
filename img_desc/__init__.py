...
def creating_session(subsession: Subsession):
    ...
    try:
        ...
        for idx, r in enumerate(rows):
            ...
            # --- FIX: LOOK FOR 'd111' AS EXPERIMENT NUMBER ---
            exp_num = 0
            # We look for "d111", "Exp", "Experiment", "Exp.", etc.
...
class FinalForProlific(Page):
    ...
    def get(self):
        ...
        if not cc: return HttpResponseRedirect(Constants.API_ERR_URL)
...
def custom_export(players):
    """
    Exports cumulative data (all sessions) in one file.
    Removed 'excel_slot' but keeps IDs.
    """
    yield [
